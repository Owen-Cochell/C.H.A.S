...
class WavePlayer(RawAudio):
    ...
    def __init__(self, chas, path=None, chunk=1024, dev=None, net=False):
        ...
        self.net = net  # Boolean value determining if we should stream audio to cleints
...
class NetAudio:
    ...
    def _gen_starter_payload(self):
        ...
        return {'id': 0,
                'data': {'format': self.format, 'channels': self.channels, 'rate': self.rate, 'chunk': self.chunk,
                         'size': self.size}}
...
class Speaker:
    ...
    def speak(self, mesg):
        ...
        process = subprocess.Popen(
            ['espeak', '-a', str(self.amplitude), '-p', str(self.pitch), '-s', str(self.rate), '-v', str(self.voice),
             str(mesg)],
